...
MCP сервер — это специальная программа, которая предоставляет инструменты для работы с сервисом Torrow через протокол Model Context Protocol. Это позволяет AI-ассистентам выполнять дополнительные действия.

Зачем это нужно?
----------------------------

Используя MCP сервер, вы можете:

* **Управлять заметками через AI-ассистента** — создавать, редактировать и искать заметки, просто общаясь с ассистентом
...
"Создай заметку 'Рецепт борща' с текстом 'Варить 2 часа' и тегами 'Рецепты' и 'Еда' в каталоге с ID abc123"
...
6. Сохраните настройки и перезапустите Cursor

7. После перезапуска проверьте, что сервер подключен — в Cursor должны появиться доступные инструменты (tools), ресурсы (resources) и промпты (prompts) от сервера Torrow.
